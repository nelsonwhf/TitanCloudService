 ...
 */
public class VerticesEdgesPipe extends AbstractPipe<Vertex, Edge> implements
		TransformPipe<Vertex, Edge> {
	...
	public VerticesEdgesPipe(final Direction direction, final int branchFactor,
			final String... labels) {
		...
		while (true) {
			if (this.nextEnds.hasNext()) {
				...
				this.nextEnds = this.doBranchFactor ?
						this.starts.next().query().direction(this.direction).labels(this.labels)
								.limit(this.branchFactor).edges().iterator() :
